...
export class TestComponent extends subscribedContainerMixin() implements OnInit {
  ...
  private onChanges(): void {
    this.trainingForm.valueChanges.subscribe(() => {
      ...
      const russianCase = formValue['russianCase'];
      if (![adjectiveId, nounId].includes('')) {
        this.text = !!russianCase ? this.giveResult(adjectiveId, nounId, russianCase, Const.adjective)
          + ' ' + this.giveResult(adjectiveId, nounId, russianCase, Const.noun) : '';
